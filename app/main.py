"""
Main FastAPI application for calculations and user management.

Defines web routes, authentication, user registration/login, and CRUD endpoints for calculations.
Handles database setup, static files, templates, and server startup.
"""
...
async def lifespan(app: FastAPI):
    """
    Application lifespan event for startup and shutdown.
...
# statis files and tempelates configuration
...
def view_calculation_page(request: Request, calc_id: str):
    """
    View calculation details page route.
    """
...
def login_json(user_login: UserLogin, db: Session = Depends(get_db)):
    """
    User login endpoint (JSON payload).
    """
...
def login_form(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    """
    User login endpoint (form payload).
    """
...
def create_calculation(
    ...
    """
    Create a new calculation for the current user.
    """
    try:
...
def list_calculations(
    ...
    """
    List all calculations for the current user.
    """
...
def get_calculation(
    ...
    """
    Retrieve a specific calculation by ID for the current user.
    """
    try:
...
def update_calculation(
    ...
    """
    Update a calculation for the current user.
    """
    try:
        calc_uuid = UUID(calc_id)
    except ValueError:
...
def delete_calculation(
    ...
    """
    Delete a calculation for the current user.
    """
    try:
        calc_uuid = UUID(calc_id)
    except ValueError:
